...
### Staleness protocol
- Both docs have a commit hash in their header comment.
- Before structural work, check staleness: `git diff --name-only <hash>..HEAD`. Use judgment — only update sections for files with structural changes (new files, changed exports/imports, new types, new constants). Skip CSS/content-only changes.
- When you make structural changes, update the affected sections and the commit hash.
