...
You MUST write ALL text in ${this.language}. Never use any other language.`,
  ...
  private applyOutcome(
    ...
    const emotionDelta: EmotionalState = kept
      ? { anger: -0.03, trust: 0.04, fear: -0.02, joy: 0.05, sadness: -0.02, curiosity: 0, guilt: -0.02 }
      : { anger: 0.06, trust: -0.05, fear: 0.02, joy: -0.04, sadness: 0.04, curiosity: 0, guilt: 0.03 };
...
function moodSummary(npc: NPC): string {
  const { anger, trust, fear, joy, sadness, curiosity, guilt } = npc.emotionalState;
  const parts: string[] = [];
